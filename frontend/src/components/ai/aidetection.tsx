...
export default function UnifiedHeroSection() {
  ...
  useGSAP(
    () => {
      ...
      // autoAlpha gracefully handles opacity + visibility hidden
      tl.to(
        hudContainerRef.current,
        {
          ...
          onReverseComplete: () => {
            setActiveFood(null); // Clear data when scrubbing back up
            if (hudContainerRef.current)
      ...
      tl.to({}, { duration: 2.5 }); // Buffer space at end of scroll
    },
    { scope: triggerRef },
  );
          ...
          {activeFood && (
            <div className="absolute inset-0 z-30 pointer-events-none">
              <svg className="absolute inset-0 w-full h-full">
                ...
                <path
                  d={`M ${activeFood.cardPos.x}% ${activeFood.cardPos.y}% L ${activeFood.anchorPos.x}% ${activeFood.cardPos.y}% L ${activeFood.anchorPos.x}% ${activeFood.anchorPos.y}%`}
                  fill="none"
                  stroke="#A9EDD2"
                  strokeWidth="1"
                  strokeDasharray="4 2"
              ...
              {/* Data Widget */}
              <div
                className="absolute w-72 bg-black/60 backdrop-blur-md border border-[#97E7CC]/30 p-4 shadow-[0_0_30px_rgba(151,231,204,0.1)] transition-all duration-300"
                style={{
